...
$logFile = Join-Path $LogDir ("wiki-compile-{0}.log" -f (Get-Date -Format 'yyyy-MM-dd'))
...
Write-RunLog "Starting /wiki-compile. SkillPath=$SkillPath VaultPath=$VaultPath"
...
try {
  & $ClaudeExe -p '/wiki-compile' 2>&1 | Tee-Object -FilePath $logFile -Append
  $exitCode = if ($LASTEXITCODE -eq $null) { 0 } else { $LASTEXITCODE }
  Write-RunLog "Finished /wiki-compile. ExitCode=$exitCode"
  exit $exitCode
}
finally {
  if (-not [string]::IsNullOrWhiteSpace($VaultPath)) {
