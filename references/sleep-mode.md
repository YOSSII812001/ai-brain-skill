...
睡眠レポートの「最終成功」と、Windowsタスクの「最後の試行」は別の情報です。
成功後に新しい試行が失敗した場合、睡眠レポートには最後に完了した成果が残ります。
また、`ready`は待機中、`healthy`は既知の要対応がない状態を表し、最後の試行が成功した証拠ではありません。

利用者が「直近で実行されたか」「最後は成功したか」と尋ねた場合、AIは次を照合します。

- `status`のcompile / lint別の最終成功と次回予定
- Windows Task Schedulerの最終実行時刻、終了結果、次回時刻、実行されなかった回数
- 最終成功より新しい安全な技術ログの完了・失敗event

時刻は設定timezoneの現地時刻へ直して伝えます。
compile間隔またはlint日次予定に空白があれば、正常表示だけで済ませず、その期間と次回予定を明示します。
「最後に成功した実行」と「それより新しい失敗した試行」は分けて報告します。
